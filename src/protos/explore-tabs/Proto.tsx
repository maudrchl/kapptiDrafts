...
import type { ExploreTab, PodEntry } from './constants'
import {
  PAGE_META,
  LOGS,
  TRACES,
...
const LOG_TAG_COLORS: Record<string, 'green' | 'orange' | 'red' | 'purple'> = {
  info: 'green',
  warn: 'orange',
  error: 'red',
  debug: 'purple',
...
const LogsView = () => {
  ...
  const logColumns = [
    ...
    },
    {
      ...
      width: 80,
      render: (v: string) => (
        <Tag color={LOG_TAG_COLORS[v]} weight="semibold" uppercase size="sm">
          {v}
        </Tag>
      ),
    },
    {
      ...
      key: 'svc',
      width: 140,
      render: (v: string) => <Tag color="blue" mono>{v}</Tag>,
    },
    {
      title: 'Message',
      dataIndex: 'msg',
      ...
      <div className={styles.searchRow}>
        ...
        <div className={styles.filterGroup}>
          ...
          <Select
            ...
            minWidth="120px"
          />
          <Button color="secondary" icon={IconPlay}>Live tail</Button>
        </div>
      </div>
      ...
      <div className={styles.kpiRow}>
        <CounterCardGroup>
          <CounterCard
            title="Total logs"
            value="1.2M"
            trend={<TrendTag current={118} previous={100} />}
          />
          <CounterCard
            title="Errors"
            value="847"
            trend={<TrendTag current={105.2} previous={100} />}
          />
          <CounterCard
            title="Warnings"
            value="3,241"
            trend={<TrendTag current={88} previous={100} invertColor />}
          />
          <CounterCard
            title="Services"
            value="14"
            trend={<StatusTag variant="ghost" color="success">All reporting</StatusTag>}
          />
        </CounterCardGroup>
      </div>
        ...
        <div className={styles.filterGroup}>
          ...
          <Select
            options={[
              ...
              { label: 'runner-eu-west', value: 'runner-eu-west' },
      ...
      <div className={styles.kpiRow}>
        <CounterCardGroup>
          <CounterCard
            title="Traces"
            value="48.2K"
            trend={<span style={{ fontSize: 12, color: '#98a2b3' }}>Last hour</span>}
          />
          <CounterCard
            title="Avg duration"
            value="127ms"
            trend={<TrendTag current={127} previous={135} invertColor />}
          />
          <CounterCard
            title="Error rate"
            value="2.1%"
            trend={<TrendTag current={2.1} previous={1.7} />}
          />
          <CounterCard
            title="P99 latency"
            value="890ms"
            trend={<TrendTag current={890} previous={1010} invertColor />}
          />
        </CounterCardGroup>
      </div>

      {TRACES.map((t) => (
        <div key={t.key} className={styles.traceRow}>
          <div className={styles.traceHead}>
            <div className={styles.traceName}>
              <StatusTag
                variant="filled"
                color={t.status === 'error' ? 'failed' : 'success'}
              >
    ...
    <>
      ...
      <div className={styles.svcMapContainer}>
        ...
        <div className={styles.svcSidebar}>
          ...
          <Card>
            <Card.Content title="Top errors (last 15m)">
              <div className={styles.detailRow}><span className={styles.detailLabel}>screenshot-svc</span><Tag color="red" weight="semibold">3.8%</Tag></div>
              <div className={styles.detailRow}><span className={styles.detailLabel}>runner-eu-west</span><Tag color="orange" weight="semibold">1.4%</Tag></div>
              <div className={styles.detailRow}><span className={styles.detailLabel}>kappticentral</span><Tag color="orange" weight="semibold">0.2%</Tag></div>
              <div className={styles.detailRow}><span className={styles.detailLabel}>api-gateway</span><Tag color="grey" weight="semibold">0.1%</Tag></div>
            </Card.Content>
          </Card>
          ...
          <Card>
            <Card.Content title="Traffic summary">
              ...
              <div className={styles.detailRow}><span className={styles.detailLabel}>Active services</span><b>14</b></div>
            </Card.Content>
          </Card>
        </div>
      </div>
  ...
  const podColumns = [
    {
      ...
      dataIndex: 'name',
      key: 'name',
      render: (v: string) => <Tag mono maxLen={40}>{v}</Tag>,
    },
    {
      title: 'Status',
      dataIndex: 'status',
      ...
      render: (v: PodEntry['status']) => (
        <StatusTag
          variant="ghost"
          color={v === 'Running' ? 'success' : v === 'CrashLoopBackOff' ? 'failed' : 'warning'}
        >
          {v}
        </StatusTag>
      ),
    ...
    },
    {
      ...
      render: (v: number) =>
        v > 5 ? <Tag color="red" weight="semibold">{String(v)}</Tag> : <span>{v}</span>,
    },
    {
      title: 'Age',
      dataIndex: 'age',
...
/* ─── Main Proto ─── */
const VIEWS: Record<ExploreTab, () => JSX.Element> = {
  logs: LogsView,
  traces: TracesView,
  svcmap: ServiceMapView,
  k8s: KubernetesView,
}
    ...
    <div className={styles.page}>
      ...
      <aside className={styles.sidebar}>
        ...
        <div className={styles.segmented}>
          <button
            className={mode === 'run' ? styles.segBtnActive : styles.segBtn}
            onClick={() => setMode('run')}
          >
            <IconZap size={12} /> Run
          </button>
          <button
            className={mode === 'obs' ? styles.segBtnActive : styles.segBtn}
            onClick={() => setMode('obs')}
          >
          ...
          <div className={styles.contentActions}>
            {meta.actions.map((a) => (
              <Button
                ...
                icon={a.label === 'Export' ? IconDownload : undefined}
              >
                {a.label}
              </Button>
            ))}
